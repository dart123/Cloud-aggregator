...
function save_user($email, $username, $password)
{
	...
	connect();
	debug_to_file("email: $email, username: $username, password: $password");
    ...
    debug_to_file("user_id: $user_id, setting_id: $setting_id, session_id: $session_id");
    close_connection();
...
function get_token($cloud, $all = false)
{
    ...
    if ($user_id)
    {
        ...
        $result = $conn->query($sql) or die(mysqli_error($conn));
        debug_to_file("result: ".print_r($result, true));
        $result_token = array();
        close_connection();
        if ($result->num_rows > 0)
        {
